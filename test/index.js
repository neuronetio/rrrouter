...
Object.assign(Test.prototype, {
	...
	isRoute(val, obj = {}) {
		...
		if (obj.route)
			this.true(val.pattern.test(obj.route), "~~> pattern satisfies route");
		...
		if (obj.count)
			this.is(val.handlers.length, obj.count, `~~> had ${obj.count} handlers`);
	}
});

test("exports", t => {
	t.isFunction(Rrrouter, "exports a function");
	t.end();
});
...
test("internals", t => {
	...
	t.true(ctx instanceof Rrrouter, "creates new `Rrrouter` instance");
...
test("add()", t => {
	...
	t.same(out, ctx, "returns the Rrrouter instance (chainable)");
...
test("remove()", t => {
	...
	ctx.remove("GET", "/foo/:hello", noop);
	console.log(ctx.routes);
	t.same(out, ctx, "returns the Rrrouter instance (chainable)");

	console.log(" ");
	t.is(ctx.routes.length, 0, 'removed "GET /foo/:hello" route successfully');
	...
	if (hasNamedGroups) {
		...
		ctx.remove("PUT", /^[/]foo[/](?<hello>\w+)[/]?$/, noop);
		t.is(
			ctx.routes.length,
			1,
			'removed "PUT /^[/]foo[/](?<hello>\\w+)[/]?$/" route successfully'
		);
	}
...
test("all()", t => {
	...
	const ctx = new Rrrouter().all("/greet/:name", req => req.chain++);
...
test("find() w/ all()", t => {
	...
	const ctx1 = new Rrrouter().all("api", noop);
	const ctx2 = new Rrrouter().all("api/:version", noop);
	const ctx3 = new Rrrouter().all("api/:version?", noop);
	const ctx4 = new Rrrouter().all("movies/:title.mp4", noop);

	console.log('use("/api")');
	t.is(find(ctx1, "/api").handlers.length, 1, "~> exact match");
	t.is(
		find(ctx1, "/api/foo").handlers.length,
		0,
	...
	t.is(
		find(ctx4, "/movies").handlers.length,
		0,
		'~> does not match "/movies" directly'
	);
	t.is(
		find(ctx4, "/movies/narnia").handlers.length,
		0,
...
test("find() w/ use()", t => {
	...
	const ctx1 = new Rrrouter().use("api", noop);
	const ctx2 = new Rrrouter().use("api/:version", noop);
	const ctx3 = new Rrrouter().use("api/:version?", noop);
	const ctx4 = new Rrrouter().use("movies/:title.mp4", noop);
	...
	t.is(
		baz2.handlers.length,
		1,
		'~> does match "/movies/narnia.mp4/cast" loosely'
	);
